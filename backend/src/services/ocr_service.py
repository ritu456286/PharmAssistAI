...
# - `\"Medicines Prescribed\"`: Extract a list of strings, where each string is the name of a prescribed medication. Only include entries that are identifiable as actual, commonly prescribed medications. Exclude anything that resembles dosages, instructions, or non-medicine related text. If no valid medication names are found, return `None`.
...
# 1. **Medicine Name Validation:** Prioritize accuracy in identifying medications. Ignore abbreviations unless their meaning is explicitly clear and refers to a medication.
# 2. **Multi-Word Medicine Names:** Correctly identify and extract multi-word medication names (e.g., \"Amoxicillin Clavulanate\").
# 3. **Exclusion:** Exclude dosages, frequencies, and administration instructions.
# 4. **Strict JSON Output:** Return only the requested fields in valid JSON format.
# 5. **Return None:** If no valid medicines are found, return `None`.
...
# #     image_path = "108.jpg"  # Change this to your new image file
# #     res = generate(image_path)
